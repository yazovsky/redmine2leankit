...
# -*- coding: utf-8 -*-
# taken from here: http://bazaar.launchpad.net/~launchpad/lp2kanban/trunk/view/head:/src/lp2kanban/kanban.py
# 
# Anton Yazovskiy made several changes in this script, fixed bugs and added new features to support leankit2redmine project.
# Updated script was initially published here: https://github.com/yazovsky/redmine2leankit
#
import httplib2
import json
import operator
...
class LeankitConnector(object):
    def __init__(self, account, username=None, password=None, throttle=1):
        host = 'http://' + account + '.leankitkanban.com'
    ...
    def _do_request(self, action, url, data=None, handle_errors=True):
        ...
        try:
            #print "Fetching " + self.base_api_url + url + "..."
            resp, content = self.http.request(
                self.base_api_url + url, action, data, headers)
        ...
        if int(resp['status']) not in LeankitResponseCodes.SUCCESS_CODES:
            print "Error from kanban"
            pprint(resp)
            raise IOError('kanban error %s' % (resp['status']))

        response = Record(json.loads(content))
